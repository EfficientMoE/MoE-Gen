...
torch::Tensor Hetero_Attn::_attn_mode_0(py::object& PyTorch_attn_module,
    ...
    */
    ...
    // this->logger_->debug("Attention mask dtype: {}", attention_mask.dtype());
    query_states = this->d2h_engine_.tensor_on_demand_copy(query_states);
    // std::string device_str = query_states.is_cuda() ? "CUDA" : "CPU";
    // this->logger_->debug("Query States copied to host memory.{}",
    // device_str);
    ...
    // this->logger_->debug("attention_mask shape: {}",
    // get_tensor_shape(attention_mask));
